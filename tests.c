...
#include <assert.h>
#include <stdio.h>
// тесты на ошибки и сложение разных типов
...
static void test_init_matrix_double() {
    ...
    SquareMatrix* m = initMatrix(2, getDoubleTypeInfo());
    assert(m != NULL);
    assert(m->size == 2);
...
static void test_init_matrix_complex() {
    ...
    SquareMatrix* m = initMatrix(2, getComplexTypeInfo());
    assert(m != NULL);
    assert(m->size == 2);
...
static void test_add_matrix_double() {
    ...
    SquareMatrix* m1 = initMatrix(2, getDoubleTypeInfo());
    SquareMatrix* m2 = initMatrix(2, getDoubleTypeInfo());
    
    double v1 = 5.0, v2 = 7.0;
    setElement(m1, 0, 0, &v1);
    setElement(m2, 0, 0, &v2);
    
    SquareMatrix* res = addMatrices(m1, m2);
    assert(res != NULL);
...
static void test_add_matrix_complex() {
    ...
    SquareMatrix* m1 = initMatrix(1, getComplexTypeInfo());
    SquareMatrix* m2 = initMatrix(1, getComplexTypeInfo());
...
static void test_multiply_matrix_double() {
    ...
    SquareMatrix* res = multiplyMatrices(m1, m2);
...
static void test_multiply_matrix_complex() {
    ...
    SquareMatrix* m1 = initMatrix(2, getComplexTypeInfo());
    SquareMatrix* m2 = initMatrix(2, getComplexTypeInfo());
...
static void test_scalar_multiply_double() {
    ...
    SquareMatrix* m = initMatrix(1, getDoubleTypeInfo());
    ...
    SquareMatrix* res = MatrixByScalar(m, &scalar);
...
static void test_scalar_multiply_complex() {
    ...
    SquareMatrix* m = initMatrix(1, getComplexTypeInfo());
...
static void test_error_handling() {
    ...
    SquareMatrix* m = initMatrix(2, getDoubleTypeInfo());
    assert(addMatrices(m, NULL) == NULL); // Сложение с NULL
    assert(multiplyMatrices(NULL, m) == NULL); // Умножение с NULL
    assert(MatrixByScalar(NULL, NULL) == NULL); // Умножение на скаляр с NULL
    
    // Тест на несовместимые размеры
    SquareMatrix* m_diff_size = initMatrix(3, getDoubleTypeInfo());
    assert(addMatrices(m, m_diff_size) == NULL);
    assert(multiplyMatrices(m, m_diff_size) == NULL);
    freeMatrix(m_diff_size);

    // Тест на несовместимые типы
    SquareMatrix* m_diff_type = initMatrix(2, getComplexTypeInfo());
    assert(addMatrices(m, m_diff_type) == NULL);
    assert(multiplyMatrices(m, m_diff_type) == NULL);
